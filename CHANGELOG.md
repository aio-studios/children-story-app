...
- 18:18 - **Home shows your own stories (#92, Step 8 - closes #67).** A "Your recent stories" shelf carrying the six most recent saved stories, each with its cover, genre and how far it's been read. Tapping one opens it straight away: the row is already in hand, so there's no spinner and no `?story=` round trip the way the Library's cards take. "All" goes to the Library. Signed-in only - a guest's single story is already the Continue card directly above, and a shelf repeating it would be the same story shown twice.
- 18:18 - **The invented shelves now know when to leave.** "Popular this week" and "Quick stories" were always placeholder content. They stay for anyone with nothing real to show - a guest, or a signed-in user on their first visit, because a Home stripped back to an empty state is worse than one showing what the app can make - and disappear the moment there is a real story to show instead.
- 18:18 - **[scripts/verify-home-recent.mjs](scripts/verify-home-recent.mjs)** - 18-check Playwright suite over the new shelf: guest, signed-in-with-nothing, and signed-in-with-eight-stories; the six-card cap, newest-first ordering, progress on a card, the invented shelves yielding, "All", opening a card, and the shelf at dark and iPad. Rows are seeded through PostgREST rather than generated, so it costs nothing to run.

### Security

- 18:18 - **A cover could be deleted by spelling its URL differently.** `/api/delete-illustration` refuses to delete any cover a saved story still points at, but the check and the deletion disagreed about what identifies a picture: the check is exact string equality against the stored URL, while Blob's `del()` accepts several spellings of the same file (a stray `?x=1`, a `#fragment`, a `?download=1`, a differently-cased host). Any of those slipped past the check as "unreferenced" and then deleted the picture anyway, leaving a saved story with a permanently broken cover. The route now reduces the URL to one canonical form - and rejects anything that isn't one of our own cover URLs outright - before both the check and the delete ([lib/imageClient.ts](lib/imageClient.ts), [app/api/delete-illustration/route.ts](app/api/delete-illustration/route.ts)). Nine new assertions in the cover-lifecycle suite hold the line (21 checks to 30).
...
- 18:18 - **A cover landing mid-story reset how far you had read.** Covers generate in the background while the story is already on screen, and when one arrived it rewrote the saved slot without the reading position written since - so Home's Continue card dropped back to 0%, resuming reopened at the top of a story already half read, and a finished story kept offering to continue. Worst for guests, whose slot is the only copy of that progress, and now the default path since cover pictures went on by default this afternoon. `updateContinueStory` in [lib/storyHistory.ts](lib/storyHistory.ts) now carries progress and reading time forward, the same way it already carried the library row id.
- 18:18 - **Interactive stories showed as barely started no matter how far in you were.** Their per-beat saves only ever wrote story content, never progress, so the row froze at whatever the first beat inserted - visible on both the Library cards and the new Home shelf. Progress now travels on its own path per beat, as it already did for classic stories ([app/create/page.tsx](app/create/page.tsx)).
- 18:18 - **Leaving a story while it was still being saved could make the next story overwrite it.** Saving a new story takes a round trip; leaving during that window deliberately stops tracking the row, but the save coming back re-armed it - pointing the next story's writes at the previous story's row and overwriting it in place. The save now checks it is still on the same story before claiming the row.
- 18:18 - **"Popular this week" flashed before your own stories on Home.** The check skipped the moment before the app knows who is signed in, when a signed-in user is indistinguishable from a guest - precisely the flash it was written to prevent ([components/HomeScreen.tsx](components/HomeScreen.tsx)).
...
- 20:47 - **The Library screen is live (#92, Step 6)** — the first user-facing piece of accounts. `/library` is a real route (not another view in `/create`'s state machine), reached from the global nav, where **Library takes the seat Favourites used to hold**. Built to the approved design in [docs/designs/library-accounts-directions.html](docs/designs/library-accounts-directions.html), Direction A frame A1.
  - **[components/LibraryScreen.tsx](components/LibraryScreen.tsx)** — cover grid (two-up on a phone, auto-filling columns from 700px), History/Favourites tabs, a capacity meter that counts quietly to 17 and warns for the last three slots, and per-story delete behind a confirmation that names the story it is about to take. Cards show the genre, when the story was made, and how far through it you are; a story with no generated cover falls back to its genre gradient, so the grid looks deliberate rather than broken for the 90% of stories with illustrations switched off.
  - **Favourites is a labelled empty shell**, shipping for real with #55. Showing the tab now is what makes the 20-story cap feel survivable; hiding it would mean re-teaching the screen later.
  - **Tapping a card opens that story** — `/create?story=<id>` loads the row by id ([lib/useStory.ts](lib/useStory.ts)), hydrates the reader, tracks the row so progress keeps saving, and marks it opened so a later regenerate lands *beside* it instead of overwriting a story you had already read. The id is stripped from the URL once the story is in hand. A story that has since been deleted or evicted lands on Home with a reason rather than an empty reader.
  - **[components/SignInForm.tsx](components/SignInForm.tsx)** — email, "Send me a link", a "check your email" state and a 45-second resend cooldown, used inline in the guest empty state. Built as its own component because Step 7's end-of-story sheet needs exactly the same thing, and two copies would drift. **A guest can now actually sign in from the app**, not just from the test harness.
  - **The guest state is a pitch, not a locked door** — nothing on this screen is gated; it explains what an account buys (your last 20 stories, on any device) and offers the form.
  - **[components/AppNav.tsx](components/AppNav.tsx)** — Library replaces Favourites in all three nav shapes (bottom bar, landscape rail, tablet sidebar). Favourites isn't gone, it moved inside the Library as a tab, so the nav keeps five seats instead of growing a sixth.
  - **[scripts/verify-library-screen.mjs](scripts/verify-library-screen.mjs)** — 42 assertions driving the real screen as a guest and as a real signed-in user, checking the **database** after every mutation rather than trusting the UI's word for it. Rows are seeded through PostgREST rather than generated, because this suite is about the screen and 19 real generations would cost money and minutes without testing anything more.

### Changed

- 20:47 - **A story card is dated from when the story was made, not when it was last touched.** `updated_at` is bumped by every progress tick and by opening a story, so dating a card from it relabelled a two-month-old story "Today" for the crime of being read. `SavedStory` now carries `createdAt` alongside the inherited `savedAt`; the sort and the eviction order still use `updated_at`, which is what they want.
- 20:47 - **Deleting a story now takes its Continue card with it.** The local continue slot carries the library row id it mirrors, so a delete clears Home's Continue card only when it is that same story — deleting story #14 leaves the card for story #3 alone. Without this, Home kept offering to resume a story that existed nowhere.
- 20:47 - **`deleteStory` reads the cover it is about to orphan instead of trusting the caller.** A Library card can sit on screen for minutes while another tab regenerates that story and attaches a new cover; deleting from the cached URL would then destroy a cover the row no longer points at while orphaning the one it does. Same stale-value trap that replace-in-place already closed.
- 20:47 - **A failed cover lookup no longer aborts the save it precedes.** `currentCover` degrades to "nothing to clean up" rather than throwing, so a transient read error leaks a Blob worth a fraction of a cent instead of costing a regenerate its place in the library.
- 20:47 - **A failed library load now offers Try again** instead of a red line over an empty grid, and the message no longer promises a pull-to-refresh gesture the screen doesn't implement.

### Fixed

- 20:47 - **The delete confirmation is keyboard-safe** — Escape closes it, focus lands on "Keep it" rather than "Delete", and focus returns to the card's menu button on close.
- 20:47 - **The "Opening your story…" cover has a way out.** The fetch behind it has no timeout, so a request that never settled would have left a full-screen overlay with no escape but a reload.

### Added

- 19:10 - **Cover images are now deleted only when nothing references them (#92, Step 5 — closes the #46 landmine).** A saved story's cover can no longer be destroyed by leaving the screen. The rule is "unreferenced", not "not yours": a cover must outlive every story row that points at it, whoever owns them — which is also the correct answer for a guest, whose covers are referenced by no row at all, so one rule covers everybody. Ordering is fixed everywhere: remove the reference first (clear the slot, overwrite or delete the row), delete the Blob second, and only if the first succeeded.
  - **[supabase/migrations/003_cover_is_referenced.sql](supabase/migrations/003_cover_is_referenced.sql)** — a `security definer` function answering one boolean, plus a partial index on `image_url`. Definer because RLS makes a plain count unable to tell "referenced by someone else" (must refuse) from "referenced by nobody" (safe to delete) — both come back as zero rows. Granted to `anon` as well as `authenticated`, since guests orphan covers too. **Applied by hand in the SQL editor**, like 001 and 002.
  - **[lib/coverBlob.ts](lib/coverBlob.ts)** — the single place that requests a Blob delete, so the guest path and the database layer can't drift apart.
  - **[lib/storyRepo.ts](lib/storyRepo.ts)** — covers now go when their row goes: a regenerate that replaces an unread story deletes the cover it displaces, and `deleteStory` takes the cover with the row.
- 19:10 - **The library holds 20 stories and evicts the oldest past that** (`evictBeyondLimit`, fired after each save). Eviction is **silent** — the capacity meter arrives with the Library screen in Step 6. The story just saved is structurally safe from its own eviction: it was written moments ago, so migration 002's trigger puts it at the top of the `updated_at` order. Deliberately does not reuse `listStories()`, which drops rows it can't map — a corrupt row would be invisible to eviction and leave the library permanently over the cap.
- 19:10 - **[scripts/verify-cover-lifecycle.mjs](scripts/verify-cover-lifecycle.mjs)** — 21 assertions covering the whole lifecycle with **real** Blobs. Real ones matter: `deleteIllustration()` no-ops on anything outside its own path prefix, so a suite built on fake URLs passes whether the code works or not. **Deletes every story belonging to `TEST_USER_EMAIL` and generates ~$0.16 of real cover images**, so it stays pointed at a throwaway account and isn't for a watch loop.
...
- 19:24 - **Em dashes thinned out where they read as machine-written, kept where they do real work.** The rule applied: remove the em dash where it was standing in for a **colon before a list** or a **full stop between two clauses** (the lazy-connector pattern that reads as AI-written); keep it where it is a genuine rhetorical pause or a paired parenthetical, which is simply good writing. Changed: the page title and OG card title (also better SEO convention), "three layouts:", "a solo build:", "Invent a hero:", and the story-mode toggle split into two sentences. Kept: the landing hero's beat before "and no two stories", the interactive section's "— because the plot is theirs", the roadmap aside, the demo story's comic beat, the cover-failure line's reassuring pause, and the setup deck's "Swipe to explore — tap to choose".
- 19:24 - **Both story prompts now steer the model's punctuation** ([lib/storyPrompt.ts](lib/storyPrompt.ts), [lib/stepPrompt.ts](lib/stepPrompt.ts)) — previously they said nothing, so **generated stories** carried em dashes freely, and those are the real product output rather than the chrome around it. The instruction asks for em dashes **sparingly, for a real pause, never as a routine connector**, rather than banning them: routine use is the AI tell, not the punctuation mark itself, and children's prose uses it legitimately. Verified against freshly generated stories.
...
- 19:10 - **A regenerate could have deleted the cover of the story it was saving** — found by `/code-review`, in code written the same hour. A cover is on screen a full round trip before the update attaching it to its row commits, so during that window the client *and* the database both report it as unreferenced. Tapping Regenerate there would have deleted a Blob that the in-flight update then pointed a saved story at, and the server's reference check couldn't help because the reference genuinely hadn't landed. The screen no longer tries to reason about a signed-in user's covers at all: only the database layer deletes those, reading the row itself to decide.
- 19:10 - **Replace-in-place trusted a stale client copy of the cover URL.** It now reads the row's actual `image_url` immediately before overwriting it — `UPDATE ... RETURNING` hands back the *new* row, so the displaced value has to be fetched deliberately rather than assumed.
- 19:10 - **A partly-failed eviction left deleted stories on screen.** The first bad delete threw and skipped the refresh. Each row is now counted only once it is actually gone, so a partial pass still refreshes and the remainder is retried by the next save.
- 19:10 - **Routine cleanup was being rate-limited into permanent leaks.** Sharing the generation budget meant a few regenerates in a minute got a cleanup refused — and a refused cleanup orphans that Blob forever, because the row referencing it is already deleted.
...
- 19:12 - **Cover lifecycle verified end-to-end — 21/21, zero console errors (#92, Step 5).** The two that matter: a cover a saved row still points at **cannot** be deleted (the endpoint refuses it and the Blob survives), and eviction past the cap removes the oldest story **and** its cover. Existence is checked with `head()` against the Blob store, never a public HEAD — Vercel's edge cache keeps answering 200 for a deleted cover, which would have made this suite report false results in both directions.
- 19:12 - **Step 4 regression gate re-run after the Step 5 changes — 16/16.** REST calls went 10 → 11, exactly accounting for the new authoritative cover read before a replace-in-place.
- 19:12 - **`/code-review` (4 findings, all confirmed and fixed) and `/security-review` (no HIGH/MEDIUM findings).** Both design fixes made the code *smaller* — once the database is the only authority on what a row points at, the client-side cover cache and its whole class of lag bugs simply disappeared. Security review cleared the `security definer` function (one boolean, `search_path = ''`, fully qualified, explicit grants), the unauthenticated endpoint (now bounded to deleting orphans, where before it would have deleted any story's cover), and client-driven eviction (RLS scopes it to the caller's own rows).
  - **Carry-forward for Step 6:** `markCurrentStoryOpened`'s `opened` flag is unreachable today, but becomes reachable when the Library can open a story — at which point a concurrent content update could reset it to `false` and let a regenerate overwrite a story the user had read.

- 18:05 - **The story library verified end-to-end with a real signed-in session — 16/16 (#92, Step 4 complete).** [scripts/verify-library-signed-in.mjs](scripts/verify-library-signed-in.mjs) drives a real browser as a real signed-in user, then reads the rows back *through RLS with that user's own token* rather than as an admin, so what it asserts is what a user can actually see. Proves a row lands with the right title, mode, selections and prose; a regenerate replaces that row **in place** instead of stacking drafts; reading position and time reach the row; and leaving a story to create a new one produces a **second** row with the first one's text and progress untouched — the exact data-loss path found earlier the same day. Committed as a reusable regression gate, since Steps 5 and 6 both build on this layer. **It deletes every story belonging to `TEST_USER_EMAIL`, so it must stay pointed at a throwaway account.**
  - Worth recording: one assertion failed on the first run because *the test* scrolled to the top of the story before leaving, which correctly wrote `progress = 0`. Progress is deliberately **last position, not furthest** (a UAT decision, so resuming reopens where you left off). The check now captures progress immediately before leaving and asserts it survives unchanged.
