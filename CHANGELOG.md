...
- 19:24 - **Em dashes removed from all user-facing copy, and from the stories themselves.** 14 in the UI (landing hero, build/interactive/roadmap sections, page title and OG card, the setup deck blurbs, the story-mode toggle, the cover-failure line, an adventure genre blurb, and the interactive screenshot's alt text). More importantly, [lib/storyPrompt.ts](lib/storyPrompt.ts) and [lib/stepPrompt.ts](lib/stepPrompt.ts) now tell the model to use plain sentence punctuation and never an em dash, so **generated stories** stop carrying the same tell. Two reasons, both real: heavy em-dash use reads as AI-written on a portfolio piece, and em dashes are awkward punctuation for an early reader. Verified against freshly generated stories: zero em dashes, prose unchanged in character.
